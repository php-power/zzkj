...
class Ranges extends Common
{
    public function index(){
        if(Request::isAjax()) {
            ...
            $list = Db::table(config('database.prefix') . 'ranges')
                ->where('name', 'like', "%" . $key . "%")
    ...
    public function edit(){
        if(Request::isAjax()) {
            ...
            $id=input('id');
            $adInfo=db('ranges')->where(array('id'=>$id))->find();
    ...
    public function editState(){
        ...
        if(db('ranges')->where('id='.$id)->update(['open'=>$open])!==false){
    ...
    public function editTj(){
        ...
        if(db('ranges')->where('id='.$id)->update(['is_tj'=>$open])!==false){
            return ['status'=>1,'msg'=>'设置成功!'];
        }else{
            return ['status'=>0,'msg'=>'设置失败!'];
        }
    ...
    public function del(){
        db('ranges')->where(array('id'=>input('id')))->delete();
      
        return ['code'=>1,'msg'=>'删除成功！'];
    }
    public function delall(){
        $map[] =array('id','in',input('param.ids/a'));
        db('ranges')->where($map)->delete();
        $result['msg'] = '删除成功！';
        $result['code'] = 1;
        $result['url'] = url('index');
        return $result;
