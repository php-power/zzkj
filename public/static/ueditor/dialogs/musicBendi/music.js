        ...
        exec:function () {
            ...
            if($G("ImageUrl").value) {
                ...
                if (parent.current_active_135item != null && audio) {
                	...
                	if($('#autoPlay:checked').size()>0) {
                		$(audio).attr('autoplay',true);
                	}
                	else{
                		$(audio).removeAttr('autoplay');
                	}
                	if($('#controls:checked').size()>0) {
                		$(audio).attr('controls',true);
                	}
                	else{
                		$(audio).removeAttr('controls');
                	}
                	if($('#loop:checked').size()>0) {
                	...
                	var html ="<section class='135editor' style='text-align:center;'><audio "
                		+ ($('#autoPlay:checked').size()>0?' autoplay ':'' )
                		+ ($('#controls:checked').size()>0?' controls ':'' )
                		+ ($('#loop:checked').size()>0?' loop ':'' )
